...
void ShapeSorter::all_of_one_type(string type_requested)
{
	vector<int> type_vec;
	int number = 1;
	for (auto const& el : shapes)
	{
		string shape_type = el->get_type();
...
void ShapeSorter::area_descending()
{
	...
	for (auto it = std::next(area_map.end(),1); it != std::prev(area_map.begin(),1); --it)
	{
		int n = it->second;
		double a = it->first;
		size_t num_len = to_string(n).length();
		size_t word_len = to_string(a).length();
		std::cout << "      " << n << std::string(26-word_len-num_len, ' ') << a << "\n";
	}
	std::cout << "\n";
}
...
int main()
{
	...
	vector<string> shape_types = {"Triangle","Square","Rectangle","Circle"};
